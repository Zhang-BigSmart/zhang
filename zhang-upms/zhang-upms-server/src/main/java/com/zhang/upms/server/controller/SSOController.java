...
import com.zhang.common.util.StringUtil;
...
public class SSOController extends BaseController{
    ...
    // 全局会话key
    //private final static String ZHANG_UPMS_SERVER_SESSION_ID = "zhang-upms-session-id";
    // 全局会话列表
    //private final static String ZHANG_UPMS_SERVER_SESSION_IDS = "zhang-upms-session-ids";
    // code kye
    ...
    public String login(HttpServletRequest request){
        ...
        // 判断是否已登录，如果已登录，则回跳
        String code = RedisUtil.get(UpmsConstant.ZHANG_UPMS_SERVER_SESSION_ID + "_" + serverSessionId);
        // code校验值
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        String hasCode = RedisUtil.get(UpmsConstant.ZHANG_UPMS_SERVER_SESSION_ID + "_" + sessionId);
        // code校验值
        if (StringUtils.isBlank(hasCode)) {
            ...
            RedisUtil.lpush(UpmsConstant.ZHANG_UPMS_SERVER_SESSION_IDS, sessionId.toString());
            // 默认验证账号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(UpmsConstant.ZHANG_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int) (session.getTimeout() / 1000));
            // code校验值
            RedisUtil.set(UpmsConstant.ZHANG_UPMS_SERVER_CODE + "_" + code, code, (int) subject.getSession().getTimeout() / 1000);
        }
        // 回调登录前地址
        String backurl = request.getParameter("backurl");
        if (StringUtils.isBlank(backurl)) {
    ...
    public Object code(HttpServletRequest request) {
        ...
        String code = RedisUtil.get(UpmsConstant.ZHANG_UPMS_SERVER_CODE + "_" + codeParam);
